const _PM_KEY = import.meta.env.VITE_PM_KEY || '';
const _PM_BACKEND_PREFIXES = ['/api/', '/pm-catalog', '/pm-catalog-v2', '/pm-audio', '/pm-lyrics', '/pm-lyrics-alt'];

const originalFetch = window.fetch;
window.fetch = async function(url, options) {
    if (typeof url === 'string') {
        const isCapacitorNative = !!(window?.Capacitor?.isNative || window?.Capacitor?.isNativePlatform?.());
        const envUrl = import.meta.env.VITE_API_URL ? import.meta.env.VITE_API_URL.replace(/\/api\/?$/, '') : 'https://pulse-music-backend.onrender.com';
        ...
        if (url.startsWith('https://lyricsplus.prjktla.workers.dev')) url = url.replace('https://lyricsplus.prjktla.workers.dev', backendUrl + '/pm-lyrics');
        else if (url.startsWith('https://lyrics-plus-backend.vercel.app')) url = url.replace('https://lyrics-plus-backend.vercel.app', backendUrl + '/pm-lyrics-alt');

        // Silence broken endpoints
        if (url.includes('lyricsplus-seven.vercel.app') ||
            url.includes('lyrics.geeked.wtf') ||
            url.includes('/pm-lyrics-alt')) {
            return new Response(JSON.stringify({ lines: [] }), { status: 200, headers: { 'Content-Type': 'application/json' } });
        }

        // Attach API key to all backend calls
        const isBackendCall = url.startsWith(backendUrl) || 
            (url.startsWith('/') && _PM_BACKEND_PREFIXES.some(p => url.startsWith(p))) ||
            (url.includes(':8000') || url.includes(':8001') && !url.includes('/ping') && !url.includes('/health')) ||
            (url.includes(envUrl.replace(/^https?:\/\//, '')) && !url.includes('/ping') && !url.includes('/health'));
        
        if (isBackendCall && _PM_KEY) {
            options = options || {};
            options.headers = { ...(options.headers || {}), 'x-pm-key': _PM_KEY };
        }
    }
...
async function uploadCoverImage(file) {
    try {
        const response = await fetch(`https://worker.uploads.pulse music.qzz.io/${file.name}`, {
            method: 'PUT',
            headers: {
        ...
        return `https://images.pulse music.qzz.io/${await response.text()}`;
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Keep the Pulse Music backend alive by pinging it every 20 seconds
    startKeepalive();
    ...
    {
        const repo = 'https://github.com/RAHUL-0568/Pulse-Music-Releases';
        ...
        const commitLink =
            hash !== 'dev' && hash !== 'unknown'
                ...
                : hash;
        const repoLink = `<a href="${repo}" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline">pulse music-music/pulse music</a>`;
    ...
    window.pulseMusicScrobbler = scrobbler;
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#playlist-modal-save')) {
            ...
            const isSpotifyActive = document.getElementById('csv-spotify-btn')?.classList.contains('btn-primary');
            const hasSpotifyImport = isSpotifyActive && document.getElementById('spotify-url-input')?.value.trim();
            const hasJspfImport = document.getElementById('jspf-file-input')?.files.length > 0;

            if (name || hasSpotifyImport || hasJspfImport) {
                const modal = document.getElementById('playlist-modal');
                ...
                if (editingId) {
                    ...
                    if (isSpotifyActive && spotifyUrlInput.value.trim()) {
                        importSource = 'spotify_import';
                        ...
                        try {
                            ...
                            currentTrackElement.textContent = 'Fetching from Spotify...';
                            ...
                            if (!nextDataMatch) throw new Error("Could not parse Spotify playlist data");
                            
                            const data = JSON.parse(nextDataMatch[1]);
                            const entity = data?.props?.pageProps?.state?.data?.entity;
                            if (!entity || !entity.trackList) throw new Error("Playlist is empty or could not be read");
                            
                            const songs = entity.trackList.map(t => ({
                                title: t.title || t.name || "",
                                artist: (t.artists && t.artists[0] && (t.artists[0].name || t.artists[0].title)) || (t.subtitle || "")
                            })).filter(s => s.title);

                            if (songs.length === 0) throw new Error("No tracks found in playlist");
                            
                            // Try to extract playlist name if we haven't set one manually
                            if (!name) {
                            ...
                            const csvText =
                                ...
                                songs
                                    .map(
                                        (s) =>
                                            `"${s.title.replace(/"/g, '""')}","${s.artist.replace(/"/g, '""')}"`
                                    )
                                    .join('\n');
                            ...
                            if (tracks.length === 0) {
                                alert('No valid tracks found in the Spotify playlist!');
                                progressElement.style.display = 'none';
                                return;
                            }

                            console.log(`Imported ${tracks.length} tracks from Spotify`);
                            ...
                            alert(`Error importing from Spotify: ${err.message}`);
                            progressElement.style.display = 'none';
                            return;
                        } finally {
                            setTimeout(() => {
                    ...
                    } else if (isYTMActive && ytmUrlInput.value.trim()) {
    ...
    if (fontSelect) {
        const savedFont = localStorage.getItem('pulse music-font');
        if (savedFont) {
        ...
        fontSelect.addEventListener('change', (e) => {
            ...
            localStorage.setItem('pulse music-font', font);
        });
    }
    ...
    if (headerAccountBtn && headerAccountDropdown) {
        ...
        async function updateAccountDropdown() {
            ...
            if (!user) {
                headerAccountDropdown.innerHTML = `
                    <button class="btn-secondary" id="header-auth-btn" style="width: 100%;">Login / Sign Up</button>
                `;
...
function showUpdateNotification(updateCallback) {
    ...
    notification.innerHTML = `
        <div>
            ...
            <p>A new version of Pulse Music is available.</p>
        </div>

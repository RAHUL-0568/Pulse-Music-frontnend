...
function notifyAudioSourceMissing() {
    ...
    import('./downloads.js').then((m) => m.showNotification('Song is unavailable')).catch(() => {});
}
...
export class LosslessAPI {
    ...
    async enrichTracksWithAlbumDates(tracks, maxRequests = 20) {
        ...
        if (!this.constructor.enrichAlbumQueue) {
            this.constructor.enrichAlbumQueue = Promise.resolve();
        }

        return new Promise((resolve) => {
            this.constructor.enrichAlbumQueue = this.constructor.enrichAlbumQueue.then(async () => {
                ...
                for (let i = 0; i < limitedIds.length; i++) {
                    ...
                    try {
                        ...
                        if (album?.releaseDate) {
                            albumDateMap.set(id, album.releaseDate);
                        }
                    } catch (e) {
                        console.warn(`[Enrich] Failed to get album date for ${id}`, e);
                    }
                ...
                console.warn('enrichTracksWithAlbumDates queue error', e);
                resolve(tracks);
            });
        });
    }
    ...
    async enrichTracksWithAlbumCover(tracks, maxRequests = 20) {
        ...
        // Use a static queue to prevent concurrent burst requests across the app
        if (!this.constructor.enrichAlbumQueue) {
            this.constructor.enrichAlbumQueue = Promise.resolve();
        }
            ...
            this.constructor.enrichAlbumQueue = this.constructor.enrichAlbumQueue.then(async () => {
                ...
                for (let i = 0; i < limitedIds.length; i++) {
                    ...
                    try {
                        const { album } = await this.getAlbum(id);
                        if (album?.cover) {
                            coverMap.set(id, album.cover);
                        }
                    } catch (e) {
                        console.warn(`[Enrich] Failed to get album cover for ${id}`, e);
                    }
                    if (i < limitedIds.length - 1) {
                        await new Promise(r => setTimeout(r, 400));
                    }
                ...
                console.warn('enrichTracksWithAlbumCover queue error', e);
                resolve(tracks);
            });
        });
    }
    ...
    async searchTracks(query, options = {}) {
        ...
        try {
            ...
            let enrichedTracks = preparedTracks;
            if (options.enrich !== false) {
                const dateEnriched = await this.enrichTracksWithAlbumDates(preparedTracks);
                enrichedTracks = await this.enrichTracksWithAlbumCover(dateEnriched);
            }
    ...
    async searchArtists(query, options = {}) {
        ...
        try {
            ...
            let enrichedArtists = preparedArtists;
            if (options.enrich !== false) {
                enrichedArtists = await this.enrichArtistsWithPicture(preparedArtists);
            }
            const result = {
                ...normalized,
                items: enrichedArtists,
    ...
    async getAlbum(id) {
        ...
        let response;
        try {
            response = await this.fetchWithRetry(`/album/?id=${id}`);
            ...
            console.warn(`Primary getAlbum failed for ${id}, falling back to hifi.getAlbum...`, error);
            try {
                const hifiRes = await HiFiClient.instance.getAlbum(id);
                if (!hifiRes || !hifiRes.album || !hifiRes.tracks) throw new Error('Album not found in fallback');
                const fallbackAlbum = this.prepareAlbum(hifiRes.album);
                const fallbackTracks = hifiRes.tracks.map(t => this.prepareTrack(t));
                const result = { album: fallbackAlbum, tracks: fallbackTracks };
                await this.cache.set('album', id, result);
                return result;
            } catch (err) {
                console.warn(`HiFi fallback also failed for ${id}:`, err);
    ...
    async getArtistSocials(artistName) {
        ...
        try {
            ...
            const searchRes = await fetch(searchUrl, {
                headers: { 'User-Agent': 'Pulse Music/2.0.0 ( https://github.com/pulse music-music/pulse music )' },
            });
    ...
    async getArtist(artistId, options = {}) {
        // Fallback for Karan Aujla (Tidal US region-locks his profile returning 404)
        if (String(artistId) === '8018117') {
            artistId = 'jio-697691';
        }
        ...
        if (String(artistId).startsWith('jio-')) {
            const jioId = String(artistId).replace('jio-', '');
            const jioRes = await fetch(`/pm-audio/artists?id=${jioId}`);
            if (!jioRes.ok) throw new Error('JioSaavn artist not found');
            const jioJson = await jioRes.json();
            if (!jioJson.success || !jioJson.data) throw new Error('JioSaavn artist data missing');
            
            const data = jioJson.data;
            const images = data.image || [];
            const pictureUrl = images.length > 0 ? images[images.length - 1].url : null;
            
            const mapJioTrack = (item) => {
                const pArtists = item.artists?.primary || [];
                const artistStr = pArtists.length > 0 ? pArtists[0].name : data.name;
                const trackImgs = item.image || [];
                return this.prepareTrack({
                    id: `jio-${item.id}`,
                    title: item.name || item.title || 'Unknown',
                    artist: { name: artistStr, id: `jio-${pArtists.length > 0 ? pArtists[0].id : jioId}` },
                    artists: [{ name: artistStr, id: `jio-${pArtists.length > 0 ? pArtists[0].id : jioId}` }],
                    album: { title: item.album?.name || '', cover: trackImgs.length > 0 ? trackImgs[trackImgs.length - 1].url : pictureUrl },
                    duration: item.duration || 0,
                    streamReady: true,
                });
            };

            const mapJioAlbum = (item) => {
                const pArtists = item.artists?.primary || [];
                const artistStr = pArtists.length > 0 ? pArtists[0].name : data.name;
                const albumImgs = item.image || [];
                return {
                    id: `jio-${item.id}`,
                    title: item.name || item.title || 'Unknown Album',
                    artist: { name: artistStr, id: `jio-${pArtists.length > 0 ? pArtists[0].id : jioId}` },
                    cover: albumImgs.length > 0 ? albumImgs[albumImgs.length - 1].url : pictureUrl,
                    releaseDate: item.year || '',
                    numberOfTracks: item.songCount || 0
                };
            };
            ...
            const result = {
                ...
                picture: pictureUrl,
                albums: [...(data.topAlbums || []).map(mapJioAlbum), ...(data.singles || []).map(mapJioAlbum)],
                tracks: (data.topSongs || []).map(mapJioTrack),
                videos: [],
            };
    ...
    async getRecommendedTracksForPlaylist(tracks, limit = 20, options = {}) {
        ...
        if (artistMap.size < 3) {
            ...
            for (const track of tracks.slice(0, 5)) {
                try {
                    ...
                    const searchResult = await this.searchTracks(searchQuery, { signal: AbortSignal.timeout(5000), enrich: false });
        ...
        const artistPromises = artistsToProcess.map(async (artist, index) => {
            try {
                // Add staggered delay to prevent rate-limiting when fetching artists concurrently
    ...
    async getTrackMetadata(id) {
        ...
        if (String(id).startsWith('jio-')) {
            const jioId = String(id).replace('jio-', '');
            const jioRes = await fetch(`/pm-audio/songs?ids=${jioId}`);
            if (!jioRes.ok) throw new Error('JioSaavn track not found');
            const jioData = await jioRes.json();
            if (!jioData.success || !jioData.data || jioData.data.length === 0) throw new Error('JioSaavn track data missing');
            const item = jioData.data[0];
            const pArtists = item.artists?.primary || [];
            const artistStr = pArtists.length > 0 ? pArtists[0].name : 'Unknown';
            const imageList = item.image || [];
            const coverUrl = imageList.length > 0 ? imageList[imageList.length - 1].url : '';
            const track = this.prepareTrack({
                id: `jio-${item.id}`,
                title: item.name || item.title || 'Unknown',
                artist: { name: artistStr, id: `jio-${pArtists.length > 0 ? pArtists[0].id : 'unknown'}` },
                artists: pArtists.map(a => ({ name: a.name, id: `jio-${a.id}` })),
                album: { title: item.album?.name || '', cover: coverUrl },
                duration: item.duration || 0,
                streamReady: true,
                isrc: ''
            });
    ...
    async getTrackRecommendations(id) {
        ...
        try {
            ...
            console.warn('Primary recommendations failed, falling back to JioSaavn...');
            try {
                const track = await this.getTrackMetadata(id);
                if (track) {
                    const artistName = track.artist?.name || track.artists?.[0]?.name || (typeof track.artist === 'string' ? track.artist : '');
                    const query = encodeURIComponent(`${artistName} ${track.title}`);
                    const searchRes = await fetch(`/pm-audio/search/songs?query=${query}`);
                    if (searchRes.ok) {
                        const searchData = await searchRes.json();
                        if (searchData.success && searchData.data?.results?.length > 0) {
                            const jioId = searchData.data.results[0].id;
                            const suggRes = await fetch(`/pm-audio/songs/${jioId}/suggestions`);
                            if (suggRes.ok) {
                                const suggData = await suggRes.json();
                                if (suggData.success && suggData.data) {
                                    const tracks = suggData.data.map(item => {
                                        const pArtists = item.artists?.primary || [];
                                        const artistStr = pArtists.length > 0 ? pArtists[0].name : 'Unknown';
                                        const imageList = item.image || [];
                                        const coverUrl = imageList.length > 0 ? imageList[imageList.length - 1].url : '';
                                        return this.prepareTrack({
                                            id: `jio-${item.id || Math.random().toString()}`,
                                            title: item.name || item.title || 'Unknown',
                                            artist: { name: artistStr },
                                            artists: [{ name: artistStr }],
                                            album: { title: item.album?.name || '', cover: coverUrl },
                                            duration: item.duration || 0,
                                            streamReady: true,
                                        });
                ...
                console.warn('JioSaavn recommendations fallback failed:', fallbackError);
            }
            return [];
        }
    }
    ...
    async loadTurnstile() {
        ...
        this.turnstileLoadPromise = new Promise((resolve, reject) => {
            const existing = document.querySelector('script[data-pulse-music-turnstile]');
            if (existing) {
            ...
            script.async = true;
            script.defer = true;
            script.dataset.pulseMusicTurnstile = 'true';
    ...
    async getAmazonCencMp4Info(streamUrl) {
        const maxInitBytes = 512 * 1024; // 512KB for initial header parsing
...
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011" xmlns:cenc="urn:mpeg:cenc:2013" profiles="urn:mpeg:dash:profile:isoff-on-demand:2011" type="static" mediaPresentationDuration="${duration}" minBufferTime="PT1.5S">
  <Period id="0" start="PT0S" duration="${duration}">
    ...
    async fetchAmazonTrackApi(apiBaseUrl, track, amazonQuality, { forceTurnstile = false } = {}) {
        ...
        const headers = {};
        let bypassToken = amazonMusicSettings.getTurnstileBypassToken().trim();
        if (bypassToken === 'your_bypass_token_here') {
            bypassToken = '';
        }
    ...
    async getAmazonMusicStreamUrl(tidalTrackId, quality = 'LOSSLESS', options = {}) {
        try {
            ...
            const title = this.getAmazonTrackTitle(track);
            console.log(`[AmazonMusic] Fetching track from ${apiBaseUrl}/api/track for ${title}`);
            let response = await this.fetchAmazonTrackApi(apiBaseUrl, track, amazonQuality, { forceTurnstile: false });

            if (response && (response.status === 401 || response.status === 428)) {
                console.log(`[AmazonMusic] Got ${response.status}, retrying with forced Turnstile refresh`);
                this.clearAmazonTurnstileJwt();
            ...
            if (!response || !response.ok) {
                ...
                console.error(`[AmazonMusic] Amazon API failed with status ${response ? response.status : 'Unknown'}:`, errText);
                throw new Error(`Amazon API failed: ${response ? response.status : 'Unknown'}`);
            }

            const rawData = await response.json();
            const data = this.getAmazonTrackApiPayload(rawData);
            
            if (!data || !data.stream_url) {
                console.error(`[AmazonMusic] Amazon API returned no stream URL:`, rawData);
                throw new Error('Amazon API returned no stream URL');
            }
            ...
            if (decryptionKey && !mp4Info?.keyId && !options.allowCencWithoutKeyId) {
                console.error('[AmazonMusic] Missing CENC key ID. mp4Info:', mp4Info);
            ...
            const codecStr = this.getAmazonCodecString(selectedQualityInfo?.codec);
            const targetCodec =
                codecStr.includes('flac') ? 'flac-raw' : codecStr.includes('alac') ? 'alac' : 'mp4a';

            console.log('[AmazonMusic] Successfully prepared Amazon stream!', { manifestUrl, targetCodec, keyId: mp4Info?.keyId });
            ...
            console.error(`[AmazonMusic] Stream failed for track ${tidalTrackId}:`, error);
            return null;
        }
    }
    ...
    async getStreamUrl(id, quality = 'LOSSLESS') {
        ...
        // 2nd source: JioSaavn (Fastest general source)
        const artistName = track?.artist?.name || track?.artists?.[0]?.name || (typeof track?.artist === 'string' ? track.artist : '');
        if (track && track.title && artistName) {
            try {
                const query = encodeURIComponent(`${artistName} ${track.title}`);
                const jioRes = await fetch(`/pm-audio/search/songs?query=${query}`);
                if (jioRes.ok) {
                    const jioData = await jioRes.json();
                    if (jioData.success && jioData.data?.results?.length > 0) {
                        const norm = (s) => (s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
                        const reqTitle = norm(track.title);
                        // Match against every credited artist, not just the first one —
                        // JioSaavn often lists a feature as the primary artist
                        const reqArtists = [
                            track.artist?.name,
                            ...(Array.isArray(track.artists) ? track.artists.map((a) => a?.name) : []),
                            typeof track.artist === 'string' ? track.artist : null,
                        ]
                            .filter(Boolean)
                            .map(norm)
                            .filter((a) => a.length > 0);
                        const mods = ['remake', 'cover', 'karaoke', 'instrumental', 'tribute', 'slowed', 'reverb', 'remix', 'mashup', 'lofi', 'sped', 'acoustic', '8d'];
                        const hasMod = (s, m) => new RegExp(`\\b${m}\\b`, 'i').test(s);
                        const reqDur = Number(track.duration) || 0;

                        // Score every result and pick the best match instead of
                        // the first acceptable one — first hits are often remixes
                        // or different songs with similar names.
                        let bestSong = null;
                        let bestScore = -1;

                        for (const song of jioData.data.results) {
                            const jioTitle = norm(song.name);
                            const dedupe = (s) => s.replace(/(.)\1+/g, '$1');
                            const reqTitleDeduped = dedupe(reqTitle);
                            const jioTitleDeduped = dedupe(jioTitle);

                            let isValidMatch = reqTitle.includes(jioTitle) || jioTitle.includes(reqTitle) ||
                                               reqTitleDeduped.includes(jioTitleDeduped) || jioTitleDeduped.includes(reqTitleDeduped);

                            // Reject covers/remixes if the original wasn't one
                            if (isValidMatch) {
                                for (const m of mods) {
                                    if (hasMod(song.name, m) && !hasMod(track.title, m)) {
                                        isValidMatch = false;
                                        break;
                                    }
                                }
                            ...
                            // Reject when the duration clearly disagrees with the
                            // requested track — the strongest wrong-audio signal
                            const jioDur = Number(song.duration) || 0;
                            if (isValidMatch && reqDur && jioDur && Math.abs(reqDur - jioDur) > 12) {
                                isValidMatch = false;
                            }

                            // Verify artist matches loosely
                            if (isValidMatch) {
                                const jioArtists = [
                                ...
                                const artistMatch =
                                    jioArtists.some(ja => reqArtists.some(ra => {
                                        const dedupedJa = dedupe(ja);
                                        const dedupedRa = dedupe(ra);
                                        return ja.includes(ra) || ra.includes(ja) || dedupedJa.includes(dedupedRa) || dedupedRa.includes(dedupedJa);
                                    })) ||
                                    // JioSaavn sometimes embeds the artist in the song name
                                    // (e.g. "Wishes Hasan Raheem" for "Wishes")
                                    reqArtists.some(ra => {
                                        const dedupedRa = dedupe(ra);
                                        return jioTitle.includes(ra) || jioTitleDeduped.includes(dedupedRa);
                                    });
                                if (!artistMatch && jioArtists.length > 0) {
                                    isValidMatch = false;
                                }
                            }

                            if (isValidMatch && song.downloadUrl && song.downloadUrl.length > 0) {
                                let score = 0;
                                if (jioTitle === reqTitle) score += 100;
                                else if (jioTitle.includes(reqTitle)) score += 60;
                                else score += 40;
                                if (reqDur && jioDur) score += Math.max(0, 30 - Math.abs(reqDur - jioDur) * 3);
                                if (score > bestScore) {
                                    bestScore = score;
                                    bestSong = song;
                                }
                            } else if (!isValidMatch) {
                                console.warn('JioSaavn result skipped due to mismatch:', song.name, 'vs', track.title, '-', artistName);
                            }
                        }

                        if (bestSong) {
                            const bestQuality = bestSong.downloadUrl.slice().sort((a, b) => parseInt(b.quality) - parseInt(a.quality))[0];
        ...
        // 3rd source: Amazon Music (if enabled, uses Python proxy)
        if (amazonMusicSettings?.isEnabled()) {
            let amazonResult = await this.getAmazonMusicStreamUrl(id, quality, { track });
            ...
            if (amazonResult?.url) {
                ...
                if (shouldProxyAmazon) {
                    streamUrl = `${window.location.protocol}//${window.location.host}/api/decrypt-stream?url=${encodeURIComponent(amazonResult.sourceUrl || amazonResult.url)}&key=${amazonResult.decryptionKey}&codec=${amazonResult.targetCodec || 'flac'}`;
                    playbackType = []; // Treat as normal unencrypted stream
                    console.log('Routing Amazon Music CENC stream through SW Decrypter');
                } else if (amazonResult.playbackType?.includes('cenc') && !canUseNativeAmazonCenc) {
                    if (amazonResult.decryptionKey) {
                        ...
                        params.set('codec', amazonResult.targetCodec || 'flac');
        ...
        // 2nd source: native Tidal manifest lookup (only needs the track ID)
        try {
            const lookup = await this.getTrack(id, quality);
            let tidalUrl;
            if (lookup?.originalTrackUrl) {
                tidalUrl = lookup.originalTrackUrl;
            } else if (lookup?.info?.manifest) {
                tidalUrl = this.extractStreamUrlFromManifest(lookup.info.manifest);
            }
            if (tidalUrl) {
                const result = {
                    url: tidalUrl,
            ...
            console.warn('Tidal stream lookup failed:', e);
        }

        // Last resort: YouTube via backend yt-dlp fallback
        if (track && track.title && artistName) {
            try {
                const query = encodeURIComponent(`${artistName} ${track.title}`);
                const dur = Math.round(Number(track.duration) || 0);
                const res = await fetch(`/api/local-fallback?q=${query}&d=${dur}`);
                if (res.ok) {
                ...
                console.warn('Local YouTube fallback failed:', e);
            }
        }

        notifyAudioSourceMissing();
        throw new Error('Could not resolve stream URL from JioSaavn, Tidal, or YouTube fallback');
    }
        ...
        if (isVideo) {
            ...
            if (String(id).startsWith('jio-')) {
                ...
                try {
                    ...
                    if (jioRes.ok) {
                        const jioData = await jioRes.json();
                        if (jioData.success && jioData.data?.length > 0) {
                            const song = jioData.data[0];
                            if (song.downloadUrl && song.downloadUrl.length > 0) {
                                const bestQuality = song.downloadUrl.slice().sort((a, b) => parseInt(b.quality) - parseInt(a.quality))[0];
                                if (bestQuality && bestQuality.url) {
                                    jioResult = {
                                        url: bestQuality.url,
                                        rgInfo: null,
                    ...
                    console.warn('JioSaavn fetch failed in enrichTrack:', e);
                }
            }
            ...
            const externalResult = jioResult?.url ? jioResult : qobuzResult?.url ? qobuzResult : amazonResult?.url ? amazonResult : deezerResult;
            if (externalResult?.url) {
                ...
                if (!deezerResult?.url && track && track.title) {
                    const artistName = track?.artist?.name || track?.artists?.[0]?.name || (typeof track?.artist === 'string' ? track.artist : '');
                    if (artistName) {
                        try {
                            const query = encodeURIComponent(`${artistName} ${track.title}`);
                            ...
                            console.warn('JioSaavn fallback failed in enrichTrack:', e);
                        }
                    }
                }
                if (deezerResult?.url || jioResult?.url) {
                    externalProvider = deezerResult?.url ? 'deezer' : 'jiosaavn';
                    externalStreamUrl = deezerResult?.url || jioResult?.url;
                    externalSourceUrl = deezerResult?.url || jioResult?.url;
                    lookup = {
                        info: {
                            audioQuality: cleanQuality,
                            trackReplayGain: 0,

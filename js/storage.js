...
export const apiSettings = {
    ...
    _loadUserInstances() {
        ...
        try {
            const stored = localStorage.getItem('pulse music-user-api-instances-v1');
    ...
    _saveUserInstances() {
        localStorage.setItem('pulse music-user-api-instances-v1', JSON.stringify(this.userInstances));
    },
...
export const recentActivityManager = {
    STORAGE_KEY: 'pulse music-recent-activity',
    LIMIT: 10,
...
export const themeManager = {
    STORAGE_KEY: 'pulse music-theme',
    CUSTOM_THEME_KEY: 'pulse music-custom-theme',
    ...
    setTheme(theme) {
        ...
        if (theme === 'system') {
            ...
            document.documentElement.setAttribute('data-theme', isDark ? 'pulse music' : 'white');
        } else {
...
export const sidebarSettings = {
    STORAGE_KEY: 'pulse music-sidebar-collapsed',
...
if (typeof window !== 'undefined' && window.matchMedia) {
    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', (e) => {
        if (themeManager.getTheme() === 'system') {
            document.documentElement.setAttribute('data-theme', e.matches ? 'pulse music' : 'white');
        }
    });
}

export const fontSettings = {
    STORAGE_KEY: 'pulse music-font-config-v2',
    CUSTOM_FONTS_KEY: 'pulse music-custom-fonts',
    FONT_SIZE_KEY: 'pulse music-font-size',
    FONT_LINK_ID: 'pulse music-dynamic-font',
    FONT_FACE_ID: 'pulse music-dynamic-fontface',
    NOTO_FALLBACK:
    ...
    loadAppleMusicFont() {
        const APPLE_FONT_LINK_ID = 'pulse music-apple-font';
...
export const amazonMusicSettings = {
    ...
    getApiBaseUrl() {
        try {
            ...
            if (!url || url.includes('localhost') || url.trim() === '/' || url.trim() === '') {
    ...
    getTurnstileSiteKey() {
        try {
            const key = localStorage.getItem(this.TURNSTILE_SITE_KEY) || import.meta.env.VITE_AMAZON_TURNSTILE_SITE_KEY;
            if (key && key !== 'your_site_key_here') {
                return key;
            }
